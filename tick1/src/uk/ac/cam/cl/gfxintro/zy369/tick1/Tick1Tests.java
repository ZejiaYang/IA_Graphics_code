...
public class Tick1Tests {
	...
	private void runTestsPart2() {
		...
		start("plane intersection: ray not at the origin");
		{
			Plane p = new Plane(new Vector3(0, 0, 0), new Vector3(0, 0, 1), new ColorRGB(0));
			Ray r = new Ray(new Vector3(0, 0, 1), new Vector3(0, 0, -1));
			check(p.intersectionWith(r).getObjectHit() == p && p.intersectionWith(r).getLocation().equals(new Vector3(0, 0, 0)));
		}
		finish();
		...
		start("plane intersection: intersect at correct location");
		{
			...
			check(p.intersectionWith(r).getLocation() != null
					&& p.intersectionWith(r).getLocation().equals(new Vector3(0, 0, 0)));
		}
		finish();
