		...
		for (int i = 0; i < elements.getLength(); ++i) {
			...
			case "sphere":
				Sphere sphere = new Sphere(getPosition(element), getDouble(element, "radius", 1), getColour(element),
						...
						getTransmittance(element),  getString(element, "texture-map"));
				scene.addObject(sphere);
				break;
				
			case "bumpy-sphere":
				BumpySphere bumpySphere = new BumpySphere(getPosition(element), getDouble(element, "radius", 1), getColour(element), getString(element, "bump-map"), getString(element, "texture-map"));
